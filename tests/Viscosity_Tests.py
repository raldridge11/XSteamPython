# -*- coding: utf-8 -*-
'''
Unit tests for Viscosity functions
'''
import unittest
...
class Test_my_ps(unittest.TestCase):

    def tearDown(self):
        stm.englishUnits = False

    def test_my_ps(self):
        pressure, entropy, viscosityCompare = Data.getTwoDimensionalTestData('SIUnits_my_ps.npz')
        viscosity = Data.calculatePropertyFromTwoDimensions(stm.my_ps, pressure, entropy)
        np.testing.assert_array_almost_equal(viscosity, viscosityCompare, decimal=2)

    def test_my_ps_English(self):
        stm.englishUnits = True
        pressure, entropy, viscosityCompare = Data.getTwoDimensionalTestData('EnglishUnits_my_ps.npz')
        viscosity = Data.calculatePropertyFromTwoDimensions(stm.my_ps, pressure, entropy)
        np.testing.assert_array_almost_equal(viscosity, viscosityCompare, decimal=2)

    def test_my_ps_error(self):
        self.assertAlmostEqual(stm.my_ps(-1.0, -1.0), 2015.0, places=2)
